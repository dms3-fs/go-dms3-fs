...
// package fuse/dms3ns implements a fuse filesystem that interfaces
// with dms3ns, the naming system for dms3-fs.
package dms3ns
...
func init() {
	if os.Getenv("DMS3FS_FUSE_DEBUG") != "" {
...
var log = logging.Logger("fuse/dms3ns")

// FileSystem is the readwrite DMS3NS Fuse Filesystem.
...
func NewFileSystem(dms3fs *core.Dms3FsNode, sk ci.PrivKey, dms3fspath, dms3nspath string) (*FileSystem, error) {

	kmap := map[string]ci.PrivKey{
		"local": sk,
	}
	root, err := CreateRoot(dms3fs, kmap, dms3fspath, dms3nspath)
	if err != nil {
		return nil, err
	}

	return &FileSystem{Dms3Fs: dms3fs, RootNode: root}, nil
}
...
type Root struct {
	...
	// Used for symlinking into dms3fs
	Dms3FsRoot  string
	Dms3NsRoot  string
...
func dms3nsPubFunc(dms3fs *core.Dms3FsNode, k ci.PrivKey) mfs.PubFunc {
	return func(ctx context.Context, c *cid.Cid) error {
		return dms3fs.Namesys.Publish(ctx, k, path.FromCid(c))
	}
}

func loadRoot(ctx context.Context, rt *keyRoot, dms3fs *core.Dms3FsNode, name string) (fs.Node, error) {
	p, err := path.ParsePath("/dms3ns/" + name)
	if err != nil {
		log.Errorf("mkpath %s: %s", name, err)
		return nil, err
	}

	node, err := core.Resolve(ctx, dms3fs.Namesys, dms3fs.Resolver, p)
	switch err {
	case nil:
	...
	root, err := mfs.NewRoot(ctx, dms3fs.DAG, pbnode, dms3nsPubFunc(dms3fs, rt.k))
	if err != nil {
		return nil, err
	}
...
func CreateRoot(dms3fs *core.Dms3FsNode, keys map[string]ci.PrivKey, dms3fspath, dms3nspath string) (*Root, error) {
	...
	for alias, k := range keys {
		...
		fsn, err := loadRoot(dms3fs.Context(), kr, dms3fs, name)
		if err != nil {
			return nil, err
		}
	...
	return &Root{
		Dms3Fs:       dms3fs,
		Dms3FsRoot:   dms3fspath,
		Dms3NsRoot:   dms3nspath,
		Keys:       keys,
		LocalDirs:  ldirs,
		LocalLinks: links,
		Roots:      roots,
...
func (s *Root) Lookup(ctx context.Context, name string) (fs.Node, error) {
	...
	// other links go through dms3ns resolution and are symlinked into the dms3fs mountpoint
	dms3nsName := "/dms3ns/" + name
	resolved, err := s.Dms3Fs.Namesys.Resolve(s.Dms3Fs.Context(), dms3nsName)
	if err != nil {
		log.Warningf("dms3ns: namesys resolve error: %s", err)
		return nil, fuse.ENOENT
	}

	segments := resolved.Segments()
	if segments[0] == "dms3fs" {
		p := path.Join(resolved.Segments()[1:])
		return &Link{s.Dms3FsRoot + "/" + p}, nil
	}

	log.Error("Invalid path.Path: ", resolved)
	return nil, errors.New("invalid path from dms3ns record")
}
...
func (fi *FileNode) Attr(ctx context.Context, a *fuse.Attr) error {
	...
	if err != nil {
		...
		return fmt.Errorf("fuse/dms3ns: failed to get file.Size(): %s", err)
	}
...
type dms3nsRoot interface {
	fs.Node
	fs.HandleReadDirAller
	fs.NodeStringLookuper
}

var _ dms3nsRoot = (*Root)(nil)

type dms3nsDirectory interface {
	fs.HandleReadDirAller
	fs.Node
	fs.NodeCreater
	fs.NodeMkdirer
...
var _ dms3nsDirectory = (*Directory)(nil)

type dms3nsFile interface {
	fs.HandleFlusher
	fs.HandleReader
	fs.HandleWriter
	fs.HandleReleaser
...
type dms3nsFileNode interface {
	fs.Node
	fs.NodeFsyncer
	fs.NodeOpener
}

var _ dms3nsFileNode = (*FileNode)(nil)
var _ dms3nsFile = (*File)(nil)
