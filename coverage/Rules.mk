...
ifneq ($(DMS3FS_SKIP_COVER_BINS),1)
...
# unit tests coverage
UTESTS_$(d) := $(shell go list -f '{{if (len .TestGoFiles)}}{{.ImportPath}}{{end}}' $(go-flags-with-tags) ./... | grep -v go-dms3-fs/vendor | grep -v go-dms3-fs/Godeps)
...
ifneq ($(filter coverage%,$(MAKECMDGOALS)),)
	...
	DEPS_test/sharness += cmd/dms3fs/dms3fs-test-cover $(d)/coverage_deps $(d)/dms3fs
endif

export DMS3FS_COVER_DIR:= $(realpath $(d))/sharnesscover/

$(d)/sharness_tests.coverprofile: export TEST_NO_PLUGIN=1
$(d)/sharness_tests.coverprofile: $(d)/dms3fs cmd/dms3fs/dms3fs-test-cover $(d)/coverage_deps test_sharness_short
