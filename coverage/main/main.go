...
func main() {
	coverDir := os.Getenv("DMS3FS_COVER_DIR")
	if len(coverDir) == 0 {
		fmt.Println("DMS3FS_COVER_DIR not defined")
		os.Exit(1)
	}
	...
	p := exec.Command("dms3fs-test-cover", args...)
	p.Stdin = os.Stdin
	p.Stdout = os.Stdout
	p.Stderr = os.Stderr
	p.Env = append(os.Environ(), "DMS3FS_COVER_RET_FILE="+retFile.Name())
