// cmd/dms3fs implements the primary CLI binary for dms3fs
package main
...
import (
    ...
    corecmds "github.com/dms3-fs/go-dms3-fs/core/commands"
    corehttp "github.com/dms3-fs/go-dms3-fs/core/corehttp"
    loader "github.com/dms3-fs/go-dms3-fs/plugin/loader"
...
var log = logging.Logger("cmd/dms3fs")
...
const (
	EnvEnableProfiling = "DMS3FS_PROF"
	cpuProfile         = "dms3fs.cpuprof"
	heapProfile        = "dms3fs.memprof"
)
...
func checkDebug(req *cmds.Request) {
	...
	if debug || os.Getenv("DMS3FS_LOGGING") == "debug" {
		u.Debug = true
...
// command ought to be executed on an dms3fs daemon.
//
...
func commandShouldRunOnDaemon(details cmdDetails, req *cmds.Request, cctx *oldcmds.Context) (http.Client, error) {
	...
	if client != nil {
		if details.cannotRunOnDaemon {
			...
			if daemonLocked, _ := fsrepo.LockedByOtherProcess(cctx.ConfigRoot); daemonLocked {
				return nil, cmds.ClientError("dms3fs daemon is running. please stop it to run this command")
			}
			return nil, nil
		}
	...
	if details.cannotRunOnClient {
		return nil, cmds.ClientError("must run on the dms3fs daemon")
	}
...
var apiFileErrorFmt string = `Failed to parse '%[1]s/api' file.
	error: %[2]s
If you're sure go-dms3-fs isn't running, you can just delete it.
`
var checkDMS3FSUnixFmt = "Otherwise check:\n\tps aux | grep dms3fs"
var checkDMS3FSWinFmt = "Otherwise check:\n\ttasklist | findstr dms3fs"
...
func getAPIClient(ctx context.Context, repoPath, apiAddrStr string) (http.Client, error) {
	...
	switch {
	case osh.IsUnix():
		apiErrorFmt = apiFileErrorFmt + checkDMS3FSUnixFmt
	case osh.IsWindows():
		apiErrorFmt = apiFileErrorFmt + checkDMS3FSWinFmt
	default:
