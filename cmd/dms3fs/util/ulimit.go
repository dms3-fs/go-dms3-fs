...
// maxFds is the maximum number of file descriptors that go-dms3-fs
// can use. The default value is 1024. This can be overwritten by the
// DMS3FS_FD_MAX env variable
var maxFds = uint64(2048)

// setMaxFds sets the maxFds value from DMS3FS_FD_MAX
// env variable if it's present on the system
func setMaxFds() {
	// check if the DMS3FS_FD_MAX is set up and if it does
	// not have a valid fds number notify the user
	if val := os.Getenv("DMS3FS_FD_MAX"); val != "" {

		fds, err := strconv.ParseUint(val, 10, 64)
		if err != nil {
			log.Errorf("bad value for DMS3FS_FD_MAX: %s", err)
			return
		}
...
// of the process based on the DMS3FS_FD_MAX value
