...
$(TGTS_GX_$(d)):
	go build -i $(go-flags-with-tags) -o "$@" "$(call dms3gx-path,$(notdir $@))"

TGTS_$(d) += $(TGTS_GX_$(d))

# multihash is special
$(d)/multihash:
	go build -i $(go-flags-with-tags) -o "$@" "dms3gx/ipfs/$(shell dms3gx deps find go-multihash)/go-multihash/multihash"
TGTS_$(d) += $(d)/multihash

# cid-fmt is also special
$(d)/cid-fmt:
	go build -i $(go-flags-with-tags) -o "$@" "dms3gx/ipfs/$(shell dms3gx deps find go-cidutil)/go-cidutil/cid-fmt"
TGTS_$(d) += $(d)/cid-fmt
